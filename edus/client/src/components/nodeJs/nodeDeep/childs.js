        ...
        <Grid item xs={10}>
          <Paper className={classes.paper}>
            <List>
              ...
              <ul>
                All instances of the ChildProcess object extend EventEmitter.
                ...
                <li><b>child.stdin :</b> Is a WritableStream corresponding to the child's standard in.</li>
                <li><b>child.stdout :</b> Is a ReadableStream corresponding to the child's standard out.</li>
                ...
                <li><b>child.kill :</b> Tries to terminate a child process, sending it an optional signal. If no signal is specified, the
                  default is SIGTERM. While the method name sounds terminal, it is not guaranteed to kill a process—it only sends  a signal
                  to a process. Dangerously, if kill is attempted on a process that has already exited, it is possible that another process
                  that has been newly assigned the PID of the dead process will receive the signal, with indeterminable consequences. This
                  method should fire a close event, which the signal used  to close the process.</li>
              ...
              <p>
                Node.js allows single-threaded, non-blocking performance but running a single thread in a CPU
                cannot handle increasing workload hence the child_process module can be used to spawn child
              ...
              <ul>
                This powerful command allows a Node program to start and interact with processes spawned via system commands. In the
                preceding example, we used spawn to call  a native OS process, ls, passing that command the lh and . arguments. In this
                way, any process can be started just as one might start it via a command line. The method takes three arguments:
              ...
              <p>
                ...
                child process can communicate with each other via send(). The fork() allows separation of
                computation-intensive tasks from the main event loop. Child processes are independent of the
                parent except the IPC communication channel established between them. Each process has its own
                memory, therefore invoking a large number of child processes can affect the performance of the
                application. The shell option is not supported by child_process.fork().
              </p>
              <ul>
              ...
              Node.js runs single threaded programming, which is very memory efficient, but to take advantage of computers multi-core systems, the Cluster module
              allows to create child processes that each runs on their own single thread, to handle the load.
              <br />
              <br />
              <ul>
                <li>
              ...
              <ul>
                <li>Single-threaded, non-blocking performance in Node.js works great for a single process. But one process in one CPU is not to be enough to handle the increasing workload of an application.</li>
                <li>No matter how powerful our server may be, a single thread can only support a limited load.</li>
                <li>We can easily spin a child process using Node’s child_process module and those child processes can easily communicate with each other with a messaging system.</li>
                <li>The child_process module enables us to access OS functionalities by running any system command inside a, well, child process.</li>
                <li>Every child process also gets the three standard stdio streams, which we can access using <b>child.stdin, child.stdout,</b> and <b>child.stderr</b>.</li>
                <li>When those streams get closed, the child process that was using them will emit the close event. This close event is different than the exit event because multiple child processes might share the same stdio streams and so one child process exiting does not mean that the streams got closed.</li>
                <li>Since all streams are event emitters, we can listen to different events on those stdio streams that are attached to every child process. Unlike in a normal process though, in a child process, the stdout/ stderr streams are readable streams while the stdin stream is a writable.</li>
              </ul>
              <br />

              <h3>Scaling node js using cluster</h3>
              <ul>
                <li>Single-threaded works fine for a single process. But eventually, one process in one CPU is not sufficient to handle the workload of our application. So, how increas scalability of the node js and how it will handle thousands of users using the application at the same time.</li>
                <li>So to handle such large loads we cant take the advantage of multi-core systems, so we can use a cluster of Node.js processes to handle the load.</li>
                <li>What cluster is, it creates the copies of our programme and distributes among the process available which means if have four core machine then it will create the four copies of the programme and provides to each core. The cluster module allows easy creation of child processes that all share server ports.</li>
