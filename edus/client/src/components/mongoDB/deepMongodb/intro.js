...
const Create_DB = `use EmployeeDB
> db.dropDatabase()
...
const findOne = `db.store.findById(_id);
...
const Filtering = `> db.store.find({name:'mukesh'});
...
db.store.update({_id: 1}, { $set: { name: ["Vocals", "Violin", "Octapad"] }})
...
const match = `db.user.aggregate({ $match: { founded_year: { $gte: 2010 } }})`.trim()

const group = `db.user.aggregate({ $group :{_id:ObjectId("5ef64c903da2b374c85626a1"), count:{
  $sum:1}} })`.trim()

const sort = `db.user.aggregate({ $sort : {name:-1} })
`.trim()

const unwind = `db.user.aggregate({ $project : {
                          author : 1 ,
                          title : 1 ,
                          tags : 1
                          }},
...
const rename = `db.posts.update({ title: 'Post Two' },{ $rename: { likes: 'views' }});`.trim()
...
<br/>
      ...
      <b>Filtering</b>
      <br/>
      It is also possible to filter your results by giving or adding some specific criteria in which you are
interested to.
      <div style={titles}>
      <PrismCode
        code={Filtering}
